...
def send(soc, message):
    ...
    """
    message = Convert_json.convert_json(message)
    taille = struct.pack('>I', len(message))
    soc.sendall(taille+message)

# Envoi de "test" sur la socket (vers le serveur) toutes les secondes
playload = ""
type = "GET_CHECK"
send(soc, (type, playload).encode())
playload = ""
type = "GET_DEPARR"
send(soc, (type, playload).encode())
playload = ""
type = "GET_MAP"
send(soc, (type, playload).encode())
playload = int(input("Entrez votre statut : joueur 0 / spectateur 1"))
type = "YOPYOP"
send(soc, (type, playload).encode())
playload = ""
type = "GO"
